...
SeqListPars* make_inpars_node(ASTNodeIt* ast)
{
    ...
    else if(ast->node->u.n->children->node->u.l->leaf_symbol->tokenName==TK_REAL)
    {
        n->t->is_record=0;
        n->t->u.pri_type=1;
    }

    else if(ast->node->u.n->children->node->u.l->leaf_symbol->tokenName==TK_RECORDID)
    {
        n->t->is_record=1;
        n->t->u.rec_id=ast->node->u.n->children->node->u.l->leaf_symbol->u.lexeme;
    }
...
SeqListPars* make_outpars_node(ASTNodeIt* ast)
{
    SeqListPars* n=(SeqListPars*) malloc(sizeof(SeqListPars));
    n->out_flg=true;
    ...
    while(1)
    {
        ...
        if(!temp->node->is_leaf)
        {
            ...
            else if(temp->node->u.n->tag_info==TAG_OUTPUT_PARS)
            {
                ...
                while(temp_ast!=NULL)
                {
                    temp_seq->next=make_inpars_node(temp_ast);
                    temp_ast=temp_ast->next;
            ...
            else if( temp->node->u.n->tag_info==TAG_TYPEDEFS)
            {
                if(temp->node->parent->node->u.n->tag_info!=TAG_MAIN)
                {
                    ...
                    func_elem->u.out_table->out_pars=make_outpars_node(temp_ast);
                    ...
                    while(temp_ast!=NULL)
                    {
                        temp_seq->next=make_outpars_node(temp_ast);
                        // func_elem->u.out_table->out_pars->out_check->ret_par=temp_ast->node->u.n->leaf_symbol->u.lexeme;
                        temp_ast=temp_ast->next;
                        temp_seq=temp_seq->next;
                    }
        ...
        while(temp1!=NULL)
        {
            for(j=0;j<LEN_HT;j++)
            {
                ...
                while(temp2!=NULL)
                {
                    ...
                    {
                        printf("%s\t%s\t%s\t%d\n", temp2->str, TypeString(temp2->ele->u.s->t->u.pri_type), temp1->str, temp2->ele->u.s->offset);
                    }
    ...
    for(i=0;i<LEN_HT;i++)
    {
        ...
        while(temp!=NULL)
        {
            if(!temp->ele->u.g->is_record)
            {
                ...
                {
                    printf("%s\t%s\t--\t%d\n", temp->str, TypeString(temp->ele->u.g->u.t->u.pri_type), temp->ele->u.g->offset);
                }
